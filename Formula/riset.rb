class Sunproject < Formula
  include Language::Python::Virtualenv

  desc "Change macOS wallpapers at sunrise/sunset"
  homepage "https://github.com/eataturk/Wallpaper-Sunrise-Sunset"
  url "https://github.com/YOURNAME/Wallpaper-Sunrise-Sunset/archive/refs/tags/v0.2.0.tar.gz"
  sha256 "TODO_SHA256_FROM_STEP_BELOW"
  license "MIT"

  depends_on "python@3.12"

  resource "astral" do
    url "https://files.pythonhosted.org/packages/.../astral-3.2.tar.gz"
    sha256 "0019dfc4b32d63c1392aa264aed2253c1e0c2fb09216f8e2cc269bbfb8bb49b5"
  end

  resource "pytz" do
    url "https://files.pythonhosted.org/packages/.../pytz-2024.1.tar.gz"
    sha256 "0019dfc4b32d63c1392aa264aed2253c1e0c2fb09216f8e2cc269bbfb8bb49b5"
  end

  def install
    virtualenv_install_with_resources
    pkgshare.install "scripts", "assets"
  end

  def post_install
    ENV["SUNPROJECT_SCRIPTS_DIR"] = pkgshare/"scripts"
    system bin/"sunproject", "post_install"
  end

  test do
    assert_match version.to_s, shell_output("#{bin}/sunproject --version")
  end
end
